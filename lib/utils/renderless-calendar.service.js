...
const monthsEnum = {
  [VIEW_MODE_SINGLE]: 1,
  [VIEW_MODE_DOUBLE]: 2,
  [VIEW_MODE_INFINITE]: 12
};

export function generateCalendarViewData(yearNumber, monthNumber, viewMode) {
  const months = monthsEnum[viewMode];

  const result = [];
  const endMonth = monthNumber + months;
  let it = monthNumber;
  let currentYear = yearNumber;
